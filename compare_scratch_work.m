...
eigenMatrix_TEM = zeros(M*(N-2), M*(N-2), 'single'); %the M(N-2) x M(N-2) matrix we need to solve
...
for i = 1:L
    for j = 1:W
        alpha_TEM(i,j) = (n_TEM(i,j)*dx*k_0)^2 - 4; 
    end
end
...
for j = 2:(N-1) %i and j are swapped ... ends up being wrong why?
    for i = 1:M %these range limits are determined by equations 4 and 5 in the documentation
        ...
        eigenMatrix_TEM(diag_index, diag_index) = alpha_TEM(i,j);
    end
end
...
for i = 1:eigenMatrix_dimension-1
    eigenMatrix_TEM(i, i + 1) = 1;
    eigenMatrix_TEM(i + 1, i) = 1;
    i;
end
...
for i = 1:eigenMatrix_dimension-M
    eigenMatrix_TEM(i, i + M) = 1;
    eigenMatrix_TEM(i + M, i) = 1;
    i;
end

disp('Finding eigenvalues/eigenvectors')
[e_vects_TEM, e_vals_TEM] = eig(eigenMatrix_TEM);
e_vals_TEM = transpose(diag(e_vals_TEM));
e_vects_back_up = transpose(e_vects_TEM);
e_vals_back_up = e_vals_TEM;

pos_e_vals_TEM = find(e_vals_TEM > 0);
fprintf('Amount of positive e_vals: %d\n', length(pos_e_vals_TEM));
disp('Done finding eigenvectors/eigenvalues')
 
%Delete negative eigenvalues and their correstponding eigenvectors
e_vects_TEM(:, find(e_vals_TEM < 0)) = [];
e_vals_TEM(e_vals_TEM < 0) = [];

%Construct the U matrix using one of the eigenvectors
%Let V be some eigenvector corresponding to a positive eigenvalue
n_eff_TEM = sqrt(e_vals_TEM) ./ (k_0 * dx);
n_eff_backup_TEM = n_eff_TEM;

%**************************************************************************
%Extract Edge Values of e_vects matrix. 
%Do this by selecting the rows which correspond to the edges
row_length = length(pos_e_vals_TEM); %number of columns
edge_values_TEM = zeros(1, row_length);
